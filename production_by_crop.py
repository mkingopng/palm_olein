import pandas as pd
import plotly.graph_objects as go
import os

DATA = 'data'

vegetable_oil_production = pd.read_csv(
    os.path.join(DATA, 'vegetable-oil-production.csv'))

year = vegetable_oil_production['Year'].drop_duplicates(
    keep='first', inplace=False)

vegetable_oil_production = vegetable_oil_production.groupby('Year').sum(
    numeric_only=True)

soybean = vegetable_oil_production[
    'Crops processed - Oil, soybean - 237 - Production - 5510 - tonnes']
sesame = vegetable_oil_production[
    'Crops processed - Oil, sesame - 290 - Production - 5510 - tonnes']
linseed = vegetable_oil_production[
    'Crops processed - Oil, linseed - 334 - Production - 5510 - tonnes']
palm = vegetable_oil_production[
    'Crops processed - Oil, palm - 257 - Production - 5510 - tonnes']
rapeseed = vegetable_oil_production[
    'Crops processed - Oil, rapeseed - 271 - Production - 5510 - tonnes']
groundnut = vegetable_oil_production[
    'Crops processed - Oil, groundnut - 244 - Production - 5510 - tonnes']
cottonseed = vegetable_oil_production[
    'Crops processed - Oil, cottonseed - 331 - Production - 5510 - tonnes']
coconut = vegetable_oil_production[
    'Crops processed - Oil, coconut (copra) - 252 - Production - 5510 - tonnes']
olive_oil = vegetable_oil_production[
    'Crops processed - Oil, olive, virgin - 261 - Production - 5510 - tonnes']
safflower = vegetable_oil_production[
    'Crops processed - Oil, safflower - 281 - Production - 5510 - tonnes']
sunflower = vegetable_oil_production[
    'Crops processed - Oil, sunflower - 268 - Production - 5510 - tonnes']
maize = vegetable_oil_production[
    'Crops processed - Oil, maize - 60 - Production - 5510 - tonnes']
palm_kernel = vegetable_oil_production[
    'Crops processed - Oil, palm kernel - 258 - Production - 5510 - tonnes']

# Plot
fig = go.Figure()

# add vectors
fig.add_trace(
    go.Scatter(x=year, y=palm, name="palm", fill='tozeroy')
),  # palm the fastest growing and now biggest supplier of edible oil

fig.add_trace(
    go.Scatter(x=year, y=soybean, name="soybean", fill="tonexty")
)  # soybean, use to be #1 and

fig.add_trace(
    go.Scatter(x=year, y=sesame, name="sesame", fill="tozeroy")
),  # sesame

fig.add_trace(
    go.Scatter(x=year, y=linseed, name="linseed", fill="tonextx")
),  # linseed

fig.add_trace(
    go.Scatter(x=year, y=rapeseed, name="rapeseed", fill="toself")
),  # rapeseed

fig.add_trace(
    go.Scatter(x=year, y=groundnut, name="groundnut", fill="tonext")
),  # groundnut

fig.add_trace(
    go.Scatter(x=year, y=cottonseed, name="cottonseed", fill="tozeroy")
),  # cottonseed

fig.add_trace(
    go.Scatter(x=year, y=coconut, name="coconut", fill="tozeroy")
),  # coconut

fig.add_trace(
    go.Scatter(x=year, y=olive_oil, name="olive_oil", fill="tonexty")
),  # olive_oil

fig.add_trace(
    go.Scatter(x=year, y=safflower, name="safflower", fill="tonextx")
),  # safflower

fig.add_trace(
    go.Scatter(x=year, y=sunflower, name="sunflower", fill="toself")
),  # sunflower

fig.add_trace(
    go.Scatter(x=year, y=maize, name="maize", fill="tonext")
),  # maize

fig.add_trace(
    go.Scatter(x=year, y=palm_kernel, name="palm_kernel", fill="tozeroy")
),  # palm_kernel

# Add figure title
fig.update_layout(
    title_text="</b>Production by Crop</b>",
    title_font_size=40,
    legend_font_size=20,
    width=1000,
    height=800
)

# Set x-axis title
fig.update_xaxes(
    title_text="</b>Year</b>",
    title_font=dict(size=30, family='Verdana', color='black'),
    tickfont=dict(family='Calibri', color='white', size=25)
)

# Set y-axes titles
fig.update_yaxes(
    title_text="<b>Metric Tons</b>",
    title_font=dict(size=30, family='Verdana', color='black'),
    tickfont=dict(family='Calibri', color='white', size=25)
)

# Show plot
fig.show()
